...
ToastFuture showToast1(
  String? msg, {
  BuildContext? context,
  Duration? duration,
  Duration? animDuration,
  StyledToastPosition1? position,
  TextStyle? textStyle,
  ...
  bool? isHaveBoxShadow
}) {
  context = context ?? currentContext;
  assert(context != null);

  StyledToastTheme? toastTheme = StyledToastTheme.of(context!);

  position ??= toastTheme?.toastPositions ?? StyledToastPosition1.bottom;

  textStyle ??=
      toastTheme?.textStyle ?? const TextStyle(fontSize: 16.0, color: Colors.white);

  textPadding ??= toastTheme?.textPadding ??
      const EdgeInsets.symmetric(horizontal: 17.0, vertical: 10.0);
  isHaveBoxShadow ??= toastTheme?.isHaveBoxShadow ?? false;
  ...
  Widget widget = Container(
    margin: EdgeInsets.symmetric(horizontal: toastHorizontalMargin, vertical: 5),
    width: fullWidth
    ...
    decoration: ShapeDecoration(
      color: backgroundColor,
      shape: shapeBorder,
      shadows: isHaveBoxShadow == true ? <BoxShadow>[
                  BoxShadow(
                    color: Colors.black.withOpacity(0.55),
                    offset: const Offset( 1, 0.7),
                    blurRadius: 3,
                  ),
                ] : [],
    ),
...
class StyledToast1 extends StatefulWidget {
  ...
  final StyledToastPosition1? toastPositions;
...
class _StyledToastState extends State<StyledToast1> {
  @override
  void initState() {
    super.initState();
  }
...
class _StyledToastWidget extends StatefulWidget {
  ...
  /// Toast position.
  final StyledToastPosition1? position;
  ...
  const _StyledToastWidget({
    ...
    this.position = StyledToastPosition1.bottom,
